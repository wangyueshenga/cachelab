...
{
    //多维数组的开辟要一行行malloc
    cache_ = (cache)malloc(sizeof(cache_asso) * S);
    for (int i = 0; i < S; ++i)
    {
        cache_[i] = (cache_asso)malloc(sizeof(cache_line) * E);
        for (int j = 0; j < E; ++j)
        {
...
void update(unsigned int address)
{
    ...
    {
        if (cache_[setindex_add][i].tag == tag_add)
        {
            cache_[setindex_add][i].stamp = 0;
            ++hit_count;
            return;
        }
    }
    ...
    {
        if (cache_[setindex_add][i].valid_bits == 0)
        {
            cache_[setindex_add][i].valid_bits = 1;
            cache_[setindex_add][i].tag = tag_add;
            cache_[setindex_add][i].stamp = 0;
            ++miss_count;
            return;
        }
    ...
    for (int i = 0; i < E; ++i)
    {
        if (cache_[setindex_add][i].stamp > max_stamp)
        {
            max_stamp = cache_[setindex_add][i].stamp;
            max_stamp_index = i;
        }
    }
    cache_[setindex_add][max_stamp_index].tag = tag_add;
    cache_[setindex_add][max_stamp_index].stamp = 0;
    return;
}
...
void parse_trace()
{
    ...
    for (int i = 0; i < S; ++i)
        free(cache_[i]);
    free(cache_); // malloc 完要记得 free 并且关文件
}